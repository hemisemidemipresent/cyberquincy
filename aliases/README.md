...
-   **Alias Supergroup**: Essentially allows for `tt` to either be a map (Tricky Tracks) or a tower upgrade (Tech Terror).
    Currently there are two supergroups:
    - `Aliases.TOWER`: towers, heroes, geraldo shop
    - `Aliases.MAP`: maps and map difficulties
    these are effectively "enums" that get parsed around as arguments to provide context to disambiguate ambiguous aliases
