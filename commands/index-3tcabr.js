...
function normalizeTowers(tower_upgrades, heroes) {
    if (heroes && heroes.length >= 2) {
        throw new UserCommandError(
            `Can't have a completion with more than 1 hero`
        );
    }
...
function embedMultiple(message, valuesList, towers, title, footer = null) {
    ...
    challengeEmbed = multipleEmbedded(
        valuesList,
        towers,
        valuesList.length,
        title
...
function getTowersDisplayStrings(valuesList, towers) {
    ...
    return h.zip(towersList).map((l) => {
        return l
            .map((t) =>
                towers.map((t) => t.toLowerCase()).includes(t.toLowerCase())
                    ? `**${t}**`
                    : t
            )
            .join('\n');
    ...
    if (n > NUM_COMBOS) {
        throw new UserCommandError(
            `You asked for the ${h.toOrdinalSuffix(
                n
            )} combo but there are only ${NUM_COMBOS} listed.`
        );
    }
...
async function displayOG3TCABRFromSubsetTowers(message, towers) {
    ...
    if (matchingCombos.length == 0) {
        throw new UserCommandError(
            `There is no 3-Tower ABR CHIMPS with ${towers.join(' + ')}`
        );
        ...
        return embedMultiple(
            message,
            matchingCombos,
            towers,
            `3-Tower ABR CHIMPS with ${towers.join(' and ')}`
        );
    }
}
...
async function displayOG3TCABRFromTowers(message, towers, combo) {
    ...
    return embed(
        message,
        values,
        `3-Tower ABR CHIMPS Combo with ${titleTowers.join(' + ')}`
    );
}
...
async function succCombos() {
    ...
    await sheet.loadCells(
        `${OG_COLS.NUMBER}${START_ROW}:${OG_COLS.LINK}${END_ROW}`
    );
