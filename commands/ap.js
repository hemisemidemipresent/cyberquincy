...
module.exports = {
    ...
    execute(message, args) {
        ...
        if (args[0] == 1) {
            if (args[1] == 'd') {
                const damageembed = new Discord.MessageEmbed()
                    .setTitle('Damage')
                    .setDescription(
                ...
                const pierceembed = new Discord.MessageEmbed()
                    ...
                    .addField(
                        'Default',
                        "projectiles continue travelling along some path (usually a straight line), until either the pierce has been used up or the projectile's lifespan expires.",
                        true
                    )
                    .addField(
                        'impact',
                        'projectiles have no meaningful concept of pierce, and usually create an effect (eg an explosion) as soon as they hit something instead of dealing damage directly.',
                        true
                    )
                    .addField(
                        'Zone',
                        'no projectile, anything in range up to the pierce limit takes damage.',
                        true
                    )
                    .setFooter(
                ...
                const mainpropembed = new Discord.MessageEmbed()
                    .setTitle(
                        'which of the following letters do you not understand?'
                    )
                    .setDescription(
                    ...
                    .addField(
                        '**md**/**cd**/**fd**',
                        'MOAB-class bloon Damage/ceramic damage/fortified bloon damage',
                        true
                    )
                    ...
                    .addField(
                        '**s**',
                        'seconds per attack/cooldown/reload',
                        true
                    )
            ...
            const buffembed = new Discord.MessageEmbed()
                ...
                .addField(
                    'buffs to speed',
                    'Buffs to speed in fact work by decreasing the reload time, instead of increasing the speed directly. These are different: a 20% buff to the reload, ie multiplying it by 0.8, is equivalent to multiplying the speed by 1/0.8 = 1.25×, so a 25% buff to the speed. Numbers in in-game descriptions usually mean a reload buff, which can be slightly misleading.\nThe new reload time will be expressed as a percentage of the current reload, so a 20% buff to an attack with 1.5s reload will be written "80%s (1.2)". Another 20% buff later in the same upgrade path will be written "80%s (0.96)".'
                )
                .setFooter(
