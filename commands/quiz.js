...
module.exports ={
    ...
    execute(message,args){
        ...
        const QuestionEmbed = new Discord.RichEmbed()
            ...
            .setThumbnail(' https://vignette.wikia.nocookie.net/b__/images/4/4b/Bloonionaire.png/revision/latest?cb=20111201195001&path-prefix=bloons')
            .addField(`${item.question}`,'type the option **letter** in chat\nyou will be given **10** seconds.')
        ...
        message.channel.send(QuestionEmbed).then(() => {
            message.channel.awaitMessages(filter, { maxMatches: 1, time: 10000, errors: ['time'] })
                .then(collected => {
                    if(item.answers.toLowerCase() === collected.first().content.toLowerCase()){
                        message.channel.send('Congratulations! You got the correct answer!')                        
                    }else{
