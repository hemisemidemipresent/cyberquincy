...
async function scrapeAllTowers() {
    ...
    for (
        ...
        const versionAdded = await parseVersionAdded(
            towersSheet,
            TOWER_HEADER_ROW,
            colIndex,
            currentVersion
...
async function loadTowerChangesCells(towersSheet, currentVersion) {
    const changesTopLeft = `${VERSION_COLUMN}${getTowerChangesRowEquivalent(
        currentVersion,
        TOWER_HEADER_ROW
    )}`;
...
async function parseTowerBalances(towersSheet, currentVersion, colIndex) {
    ...
    for (
        rowIndex = TOWER_HEADER_ROW;
        rowIndex < TOWER_HEADER_ROW + currentVersion - 1;
        rowIndex++
    ) {
        ...
        const buffs =
            towersSheet.getCell(rowIndex, colIndex).note?.replace(/✔️/g, '✅')?.split('\n\n') || [];
        const nerfs = towersSheet.getCell(rowIndex, colIndex + 1).note?.split('\n\n') || [];
        const fixes =
            towersSheet
                .getCell(changesRowIndex, colIndex)
                .note?.replace(/🟡/g, '⚠️')
                ?.split('\n\n') || [];
        const changes =
            towersSheet
                .getCell(changesRowIndex, colIndex + 1)
                .note?.replace(/🟦/g, '↔')
                ?.split('\n\n') || [];
...
async function parseHeroBalances(heroesSheet, currentVersion, colIndex) {
    ...
    for (rowIndex = HERO_HEADER_ROW; rowIndex < HERO_HEADER_ROW + currentVersion - 1; rowIndex++) {
        ...
        const buffLikes =
            heroesSheet.getCell(rowIndex, colIndex).note?.replace(/✔️/g, '✅')?.split('\n\n') || [];
        const nerfLikes = heroesSheet.getCell(rowIndex, colIndex + 1).note?.split('\n\n') || [];
        const combined = buffLikes
            .concat(nerfLikes)
            .map((n) => n.replace(/🟡/g, '⚠️').replace(/🟦/g, '↔'));
...
async function loadBuffNerfCells(sheet, headerRow, currentVersion) {
    ...
    await sheet.loadCells(`${VERSION_COLUMN}${headerRow}:${bottomRightCellToBeLoaded}`);
}
...
async function parseVersionAdded(sheet, headerRow, colIndex, currentVersion) {
    ...
    for (rowIndex = headerRow; rowIndex < headerRow + currentVersion - 1; rowIndex++) {
        ...
        if (
            cellFormatting &&
            !isEqual(cellFormatting.backgroundColor, { red: 0.6, green: 0.6, blue: 0.6 })
        ) {
