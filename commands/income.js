...
module.exports = {
    ...
    execute (message, args) {
        ...
        if (!args[2]) {
            const normalStartRound = parseInt(args[0]) - 1 // thats just how it works
            const startroundObject = r[normalStartRound]
            const endroundObject = r[endround]
            const income = endroundObject.cch - startroundObject.cch
            ...
            const startround = parseInt(args[0]) - 1
            const startroundObject = abr[startround - 2] // the data works in a way that basically means that its an array of arrays, ordered by round number
            const endroundObject = abr[endround - 2]
            const income = endroundObject[1] - startroundObject[1]
            const embed = new Discord.MessageEmbed()
                .setTitle(
                    `$${
                        Math.trunc(income * 100) / 100
