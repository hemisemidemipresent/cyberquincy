...
module.exports = {
  ...
  execute(message, args, client) {
    function level_cal(B16, xp_slope, diff_mult, heroname) {
      ...
      for (i = 3; i < 22; i++) {
        r.push(r[i - 1] - ((r[i - 2] - r[i - 1]) / diff_mult + 20) * diff_mult);
      }
      for (i = 22; i < 52; i++) {
        r.push(r[i - 1] - ((r[i - 2] - r[i - 1]) / diff_mult + 40) * diff_mult);
      }
      for (i = 52; i < 102; i++) {
        //might be broken
        r.push(r[i - 1] - ((r[i - 2] - r[i - 1]) / diff_mult + 90) * diff_mult);
      }
    ...
    if (!args[0] && !args[1] && !args[2]) {
      ...
      if (args[0] == "quincy") {
        ...
        return message.channel.send("Please specify a valid hero name!");
      }
      ...
      if (args[2].includes("eas") || args[2].includes("beg") || args[2] == 1) {
        var h = 1;
      } else if (
        args[2].includes("int") ||
        args[2].includes("med") ||
