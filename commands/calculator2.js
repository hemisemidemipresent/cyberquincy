...
async function initLexer(args) {
    ...
    try {
        ...
        if (c) {
            ...
            res.error = new Discord.MessageEmbed()
    ...
    parsed.forEach(function (t) {
        switch (t) {
            ...
            case '^':
                break;
            default:
                // any of the following is in the input, a price Multiplier is required
                if (
                    isTowerUpgradeCrosspath(t) ||
                    Towers.isTowerUpgrade(Aliases.getCanonicalForm(t)) ||
                    Towers.isTower(Aliases.getCanonicalForm(t)) ||
                    Aliases.isHero(Aliases.getCanonicalForm(t))
                ) {
                    res.needMode = true;
                }
        }
...
module.exports = {
    ...
    async execute(message, args) {
        ...
        let res = await initLexer(args);
        if (res.error) {
            return await message.channel.send(res.error);
        }
        if (res.needMode) {
        ...
        } else await calc(message, args, i);
    },
    helpMessage,
}; 
*/
