...
async function paragon_stats(interaction) {
    ...
    let costText =
        ["easy", "medium", "hard", "impoppable"]
            .map((difficulty) => `${bHelper.rawDifficultyMult(paragonCost, difficulty)} - ${difficulty}`)
            .join("\n") + "\n\n";
    desc = costText + desc;
...
async function paragon_degree(interaction) {
    ...
    if (isNaN(totalMoneySpent))
        return await interaction.reply({ embeds: [totalMoneySpent], flags: MessageFlags.Ephemeral });
...
function lex(input, operator, value, difficulty) {
    ...
    try {
        ...
        if (c) {
            ...
            if (c === "<")
                embed.setFooter({
                    text: "Did you try to tag another discord user? That's definitely not allowed here.",
                });
            return embed;
