...
const REAL_CYBER_QUINCY_USER_ID = 'FILL_ME_IN';
...
async function unsubmit(message, url) {
    ...
    if (submission = messages.get(message_id)) {
        ...
        if (submitterTag == message.author.tag) {
            ...
            return message.channel.send(`You, ${message.author.tag}, didn't submit this. ${submitterTag} did. You must be the submitter to delete.`)
        }
    } else {
...
function helpMessage() {
    return new Discord.MessageEmbed()
                ...
                .addField('`q!isub <link_to_submission_message>`', 'Removes your submission if it is still there')
