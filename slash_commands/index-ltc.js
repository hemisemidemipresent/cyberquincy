...
async function ltc(map, modifier) {
    ...
    if (!result) {
        let desc = `No LTC found for ${map}`;
        if (modifier) desc = `No ${modifier} LTC found for ${map}`;
...
async function fetchltc(searchParams) {
    ...
    if ("error" in resJson) throw new Error(resJson.error);
    return resJson;
}
