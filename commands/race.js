...
module.exports = {
    ...
    execute(message, args) {
        if (!args)
            ...
            let str = args.join(' ');
            let racenum = findrace(str, message);
        } else if (args[0] < 1)
            return message.channel.send('please specify a valid race number');
        else {
            access(parseInt(args[0]), message);
        }
    },
};
async function findrace(str, message) {
    ...
    const sheet = doc.sheetsByIndex[14]; //load race spreadsheet
    await sheet.loadCells('G3:G3'); // loads race #
    const raceNum = parseInt(sheet.getCellByA1('G3').value.slice(20));
    for (let i = 1; i <= raceNum; i++) {
        await sheet.loadCells(`C${i * 3 + 9}:C${i * 3 + 9}`); // loads race name
        let name = sheet.getCellByA1(`C${i * 3 + 9}`).value.toLowerCase(); // race name
        if (str.includes(name) || name.includes(str)) {
            return access(i, message);
        }
    }
...
async function access(n, message) {
    const { GoogleSpreadsheet } = require('google-spreadsheet');
