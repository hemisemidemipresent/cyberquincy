...
async function scrapeAll2TCCombos() {
    ...
    await sheet.loadCells(`${COLS.NUMBER}${rOffset + 1}:${COLS.CURRENT}${rOffset + nCombos}`);

    let combos = [];
    for (let n = 1; n <= nCombos; n++) {
        combos.push(parsePreloadedRow(rOffset + n));
    }
...
function parsePreloadedRow(row) {
    ...
    let values = {};
    const ogSpecificCols = Object.keys(COLS).filter(
        (col) => !['PERSON', 'MAP', 'LINK'].includes(col)
    );
...
async function findOGRowOffset() {
    ...
    await sheet.loadCells(`${COLS.NUMBER}${MIN_OFFSET}:${COLS.NUMBER}${MAX_OFFSET}`);
...
function parseMapCompletions(row) {
    ...
    const ogMapAbbr = Aliases.mapToIndexAbbreviation(Aliases.toAliasNormalForm(ogCells.MAP.value));
