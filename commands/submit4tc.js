...
module.exports = {
    ...
    async execute(message, args) {
        if (args.length < 2) {
            return message.channel.send('Use format: q!submit4tc <code> <name>');
        }
...
function get_challenge_data(challengeCode) {
    ...
    const url = 'https://static-api.nkstatic.com/appdocs/11/es/challenges/' + challengeCode;
        ...
        request(url, (useless, response, body) => {
            if (response.statusCode == 200) {
                ...
                zlib.inflate(base64Buffer, (error, zlibBuffer) => {
                    ...
                    const challengeData = JSON.parse(zlibBuffer.toString('utf8'));
...
function get_version(cD) {
    if (cD.bloonModifiers.allCamo === undefined) {return 9;}          // 9.0 - 10.2
    if (cD.numberOfPlayers === undefined) {return 11;}                // 11.0 - 11.2
    if (cD.replaces === undefined) {return 12;}                       // 12.0
    if (cD.towers[9].tower != 'Adora') {return 12.1;}                 // 12.1 - 13.1
    if (cD.towers[0].path1NumBlockedTiers === undefined) {return 14;} // 14.0 - 15.2
    if (cD.towers[10].tower != 'AdmiralBrickell') {return 16;}        // 16.0 - 17.1
    if (cD.displayIncludedPowers === undefined) {return 18;}          // 18.0 - 18.1
    if (cD.towers[11].tower != 'Etienne') {return 19;}                // 19.0 - 19.2
    if (cD.uniqueDCId === undefined) {return 20;}                     // 20.0 - 20.1
    if (cD.roundSets === undefined) {return 21;};                     // 21.0 - 21.1
...
function can_beat_6(towers, version) {
    const startTowers = ['Quincy', 'Ezili', 'DartMonkey', 'BoomerangMonkey', 'TackShooter', 'IceMonkey', 'SniperMonkey', 'MonkeySub', 'MonkeyBuccaneer', 'WizardMonkey', 'NinjaMonkey', 'Alchemist', 'Druid', 'EngineerMonkey'];
...
function can_beat_24(towers) {
    const camoTowers = ['Quincy', 'Gwendolin', 'ObynGreenfoot', 'Etienne', 'Ezili', 'DartMonkey', 'SniperMonkey', 'MonkeySub', 'MonkeyBuccaneer', 'MonkeyAce', 'HeliPilot', 'MortarMonkey', 'DartlingGunner', 'WizardMonkey', 'NinjaMonkey', 'SpikeFactory', 'EngineerMonkey'];
    ...
    if (towers.includes('MonkeyVillage')) {
        if (towers.includes('BoomerangMonkey') || towers.includes('TackShooter') || towers.includes('Alchemist') || towers.includes('Druid')) {
            return true;
        }
        if (towers.includes('IceMonkey') && (towers.includes('StrikerJones') || towers.includes('Bomb'))) {
            return true;
        }
    }
    return false;
...
function valid_settings(cD, version) {
    ...
    if (cD.difficulty != 'Hard' || cD.mode != 'Clicks' || !cD.disableSelling || bloonSpeed != 1 || moabSpeed != 1 || bloonHealth != 1 || moabHealth != 1) {
        return false;
    }
    ...
    if (!(lives == 1 || lives == -1 ) || !(cash == 650 || cash == -1) || !(start == 6 || start == -1) || !(end >= 100 || end == -1)) {
        return false;
    }
...
function remove_combos(submission) {
    const remainingCombosFilePath = 'C:/Users/roeal/source/repos/cyber_quincy_4tc/jsons/remaining_combos.json'; // relative path: '../jsons/remaining_combos.json'
    const remainingCombos = require(remainingCombosFilePath);
    ...
    fs.writeFileSync(remainingCombosFilePath, JSON.stringify(remainingCombos, null, 4), (error, data) => {
        if (error) {
            throw error;
        }
    }); 
...
function update_leaderboard(submission) {
    const leaderboardFilePath = 'C:/Users/roeal/source/repos/cyber_quincy_4tc/jsons/leaderboard.json'; // relative path: '../jsons/leaderboard.json'
    const leaderboard = require(leaderboardFilePath);
    ...
    fs.writeFileSync(leaderboardFilePath, JSON.stringify(leaderboard, null, 4), (error, data) => {
        if (error) {
            throw error;
        }
    });
...
async function submit(submission) {
    ...
    const submittedCombosFilePath = 'C:/Users/roeal/source/repos/cyber_quincy_4tc/jsons/submitted_combos.json'; // relative path: '../jsons/submitted_combos.json'
    const validatedCombos = require(submittedCombosFilePath);

    // Remove combos from submission then add it to the submitted_combos.json
    validatedCombos.push({name: submission.name, code: submission.code, towers: submission.towers});
    fs.writeFileSync(submittedCombosFilePath, JSON.stringify(validatedCombos, null, 4), (error, data) => {
        if (error) {
            throw error;
        }
    });
