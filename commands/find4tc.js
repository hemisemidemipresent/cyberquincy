const remainingCombosFilePath = 'C:/Users/roeal/source/repos/cyber_quincy_4tc/jsons/remaining_combos.json'; // relative path: '../jsons/leaderboard.json'
...
module.exports = {
    ...
    async execute(message, args) {
        ...
        const validTowers = ['Quincy', 'Gwendolin', 'StrikerJones', 'ObynGreenfoot', 'CaptainChurchill', 'Benjamin', 'Ezili', 'PatFusty', 'Adora', 'AdmiralBrickell', 'Etienne', 'DartMonkey', 'BoomerangMonkey', 'BombShooter', 'TackShooter', 'IceMonkey', 'GlueGunner', 'SniperMonkey', 'MonkeySub', 'MonkeyAce', 'HeliPilot', 'MortarMonkey', 'DartlingGunner', 'WizardMonkey', 'SuperMonkey', 'NinjaMonkey', 'Alchemist', 'Druid', 'SpikeFactory', 'MonkeyVillage', 'EngineerMonkey'];
        ...
        message.channel.send('Message too long to display, creating a hastebin link');
        const hastebinLink = await hastebin(response.found4tcs, {extension: "txt"});
...
function find4tc(towers) {
    ...
    for (let i = 0; i < remainingCombos.length; ++i) {
        if (is_subset(towers, remainingCombos[i])) {
            ...
            found4tcs += temp[0] + ', ' + temp[1] + ', ' + temp[2] + ', ' + temp[3] + '\n';
            combos += 1;
        }
    }
