...
module.exports = class AliasRepository extends Array {
        ...
        for (const canonical in nextAliases) {
            ...
            try {
                ...
                if(e instanceof AliasError) {
                    ...
                    console.log(`  |-> Refusing to add new group ${this.formatAliasGroup(nextAliasGroup)} to collection`);
                } else {
                    throw e;
                }
            }
    ...
    getAliasGroup(aliasMember) {
        var ags = this.filter(ag => 
            ag.canonical == aliasMember || 
            ag.aliases.includes(aliasMember)
        );
        if (!ags || ags.length == 0) {
            null
            ...
            throw `Multiple alises groups found sharing a given alias member` +
    ...
    allMaps() {
        ...
        const allMaps = easyMaps.concat(intermediateMaps).concat(hardMaps).concat(expertMaps);
