...
module.exports = {
	...
	execute(message, args, client) {
		...
		message.channel.awaitMessages(filter, { maxMatches: 1, time: 10000, errors: ["time"] }).then(a => {
			...
			//convert monays
			if (option == 1) {
				return message.channel.send('sorry, this option is under repair')
				...
				message.channel.awaitMessages(filter, { maxMatches: 1, time: 10000, errors: ["time"] }).then(tt => {
					...
					message.channel.awaitMessages(filter, { maxMatches: 1, time: 10000, errors: ["time"] }).then(p => {
						...
						if (path1 > 5 || path1 < 0 || path2 > 5 || path2 < 0 || path3 > 5 || path3 < 0) {
						...
						message.channel.send(`The total cost is (including base tower cost) is ${parseInt(costs[0]) + parseInt(costs[1]) + parseInt(costs[2]) + parseInt(t[tower].base.cost)} (on medium)`)
					})
				})
			}
		})
